...
Version 0.4.7 (draft), January 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted traffic over the years[^96]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   Correlation Fingerprinting Attack: As illustrated (simplified) below, this attack will fingerprint[^97] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It's able to do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][17]

-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^98] a bomb threat hoax at Harvard University.
...
Be aware again that this might not be enough against a motivated global adversary[^99] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor in itself can already be considered a suspicious activity[^100] and its use could be considered malicious by some[^101].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^102]. Most crypto currencies can be tracked accurately through various methods[^103].

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You'll have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^104] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy in mind like Monero but even those can be de-anonymized to some extent[^105][^106].

Even if you use Mixers or Tumblers (services that specialize in anonymizing crypto currencies by "mixing them"), keep in mind this is only obfuscation and not actual anonymity[^107].

## Exploits in your apps and services:

So you're using Tor Browser or Brave Browser with a Tor Tab. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^108] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^109] case in 2013 where the FBI inserted malware[^110] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^111] hack that breached several US government institutions by inserting malware into an official software update server.
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^112] without being able to access identifying information or compromise your system.

## Your CPU:

All modern CPUs[^113] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^114] and the AMD Platform Security Processor[^115].
...
These have already been affected by several security vulnerabilities in the past[^116] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^117].

There are some not so easy ways[^118] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.

If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^119].
...
These have been used since at least since 2008 using an attack called "Jasager"[^120] and can be done by anyone using self-built tools or using commercially available devices such as Wi-Fi Pineapple[^121].
...
These devices can fit in a small bag and can take over the Wi-Fi environment of any place within their range. For instance a Bar/Restaurant/Café/Hotel Lobby. These devices can force Wi-Fi clients to disconnect from their current Wi-Fi (using de-authentication, disassociation attacks[^122]) while spoofing the normal Wi-Fi networks at the same location. They will continue to perform this attack until your computer or yourself decides to try to connect to the rogue AP.

These devices can then mimic a captive portal[^123] with the exact same layout as the Wi-Fi you're trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you open access internet that they'll themselves get from the same place.
...
There are readily available commercial and cheap "badUSB" [^124]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^125] in 2005.

While you could inspect an USB key physically, Scan it with various utilities, check the various components to see if they're genuine, you will most likely never be able to discover complex malware embedded in genuine parts of a genuine USB key by a skilled adversary without advanced forensics equipment[^126].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^127], Cellebrite Cloud[^128]) that will help an adversary analyze your cloud data with ease.
...
The digital fingerprint is the way you write[^129], the way you behave[^130]. The way you click. The way you browse. The fonts you use on your browser[^131]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
This technology is also widely used in CAPTCHAS[^228] services to verify that you are "human" and can be used to fingerprint a user.
...
There are also a few cases involving OSINT at Bellingcat[^133].Have a look at their very informative toolkit here: <https://bit.ly/bcattools>
...
Your Browser and Device Fingerprints[^238] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance websites will be able to provide a "mobile experience" if you're using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^134] based browsers (such as Chrome) or Firefox[^135] unless taking special measures.
...
Companies like Facebook have used advanced face recognition for years[^136]'[^137] and have been using other means (Satellite imagery) to create maps of "people" around the world[^138].
...
Even if you're not looking at the camera, they can still figure out who you are[^141] and even make out your emotions[^142] and probably guess your political affiliation[^143].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^144]'[^145]'[^146]'[^147]'[^148].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^149]) in their CCTV networks to look for "persons of interest"[^150]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^151] including possibly identifying ethnic minorities[^152]. A simple face recognition error by some algorithm can ruin your life[^153].

Apple is making FaceID mainstream and pushing its use it to log you in in various services including the Banking systems.

Same goes with fingerprint authentication being mainstreamed by many smartphone makers to authenticate yourself. A simple picture where your fingers appear can be used to de-anonymize you[^154].

We can safely imagine a near future where you won't be able to create accounts or sign-in anywhere without providing unique biometrics (A good time to re-watch Gattaca[^155], Person of Interest[^156] and Minority Report[^157]). And you can safely imagine how useful these large biometrics databases could be to some interested third parties.

In addition, all this information can also be used against you (if you are already de-anonymized) using deepfake[^158] by crafting false information (Pictures, Videos, Voice Recordings...) and have already been used for such purposes[^159].

At this time, there are a few steps[^160] you can use to mitigate (and only mitigate) face recognition when conducting sensitive activities where CCTV might be present:

-   Wear a facemask as they have been proven to defeat some face recognition technologies[^161].
...
Phishing[^162] is a type of attack where an adversary could try to extract information from you by pretending to be something/someone else.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they're encrypted[^163]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And last but not least here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^164]
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^165] or a national security letter[^166]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^167].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^168].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^169].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^170].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^171].

Some providers have implemented the use of a Warrant Canary[^172] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Last but not least, it's now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^173] and Omnisec story[^174]).
...
There are many advanced techniques that can be used by skilled adversaries[^175] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^176] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw>
...
-   If you absolutely want system wide plausible deniability[^187] despite the risks[^177].
...
TAILS[^179] stands for **The Amnesic Incognito Live System.** . It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
-   Enable BIOS/UEFI password and use a long passphrase[^180] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^181]. Keep it on if you intend to use Linux/Windows.
...
So what is Secure Boot[^182]? In short, it's a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically when the Operating Systems (or the Bootloader[^183]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^184] (with pictures).
...
This route will make extensive use of Virtual Machines[^185], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^191] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
Cold Boot attacks[^194] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^195], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability. There have been some forensics studies[^196] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^197] and by the maintainer of Veracrypt[^198] so I wouldn't worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^200].

-   Windows Indexing keeping traces of the files present in your User folder by default[^201].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^202]'[^203].
...
-   Gatekeeper[^204] and XProtect keeping track of your download history in a local database
...
Forensics could[^195]'[^198] use all those leaks[^206] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Veracrypt[^205] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^206] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^207] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^208] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^209] Operations[^210]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Enter a strong passphrase (longer the better)[^179]
...
-   Wipe mode:
    ...
    -   If you have sensitive data on an SSD, Trim alone should take care of it[^212] but I would recommend 1 pass (random data) just to be sure.
...
-   Use a strong passphrase[^179] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^217] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^218] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

I'm aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^219] have not be completely fixed to this date[^220] and if you're using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This route will use Virtualization and Whonix[^221] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later on you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^222] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
-   Apply this fix from VirtualBox to mitigate Spectre/Meltdown vulnerabilities by running this command from the VirualBox Program Directory:

    -   VBoxManage modifyvm "Whonix-Gateway-XFCE" \--spec-ctrl on

    -   VBoxManage modifyvm "Whonix-Workstation-XFCE" \--spec-ctrl on
...
You will need something to store your data (logins/passwords, identities and TOTP[^224] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^225] authentication with the authenticator feature.
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^226], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^228] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^229] authentication with the authenticator feature.
...
Captcha[^230] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^231] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^232]) and Cloudflare (hCaptcha[^233]). hCaptcha is used on 15% of the internet by their own metrics[^234].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^235]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^236].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^237].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^239].
...
Browser and Device[^240] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^241]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^242] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^243].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^244] and unfortunately sometimes required.
...
This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^245]. Unfortunately this method is now also sometimes being used on Facebook[^246] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^247])
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^239] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^248].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^249].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^250] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^253] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^254] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^255] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^256]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^257]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
This time you will need to submit an appeal using a form[^258], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using at least seven proxies[^259] that are not known by 4chan blocking system.
...
-   Use an Android Emulator (Youwave[^260], Bluestacks[^261] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
So the fact is that it's very very unlikely[^268]'[^269] for a forensic examiner to be able to recover data from a Trim enabled SSD but it's not completely impossible either[^270]'[^271]'[^272] if they're fast enough and have access to extensive equipment and skills.
...
Obviously for maximum security, Sanitize type options should be preferred. Unfortunately are no **free** easy (bootable with a menu) all-in-one tools available and you'll be left with either going with drive manufacturers provided tools, the free manual hdparm[^273] utility or going with a commercial tool such as PartedMagic[^274] or Blancco[^275].
...
This guide will therefore recommend the use of the free utility hdparm using a GParted[^276] boot USB.
...
Unfortunately Apple has removed the secure erase options from the trash bin even for HDD drives[^278]. So you're left with using other tools:
...
The best tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^279] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux/OSX quite easily. I never managed to make it work properly within Windows due various python dependencies despite the provided instructions. It's however very straightforward to install and use on Linux. So I would suggest creating a small Debian VM within Virtualbox (behind your Whonix Gateway) and then deploy the web version following the instructions provided here: <https://0xacab.org/jvoisin/mat2-web>. Which you can then use from your other VMs to analyze various files from a convenient web interface (I do recommend you take a snapshot of this VM before use to revert the state after analysis to remove traces from the VM in question).

Mat2 is also pre-installed on the Whonix Workstation VM[^280] and available on TAILS by default[^281].

Soon I intend to provide a convenient and ready-to-use Debian (stabme) VM image with mat2-web pre-configured as an asset to this guide. This VM will work out of the box with Whonix Gateway and provide easy cross-platform access to Mat2 on OSX/Windows users.
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^206]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^198] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally I would just pick Normal Deletion + Trim (Trim itself should be enough[^210]). Secure Deletion with Trim[^207] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^282] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^283] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^284] and Secure Communication Obfuscation[^285] through Steganography[^286] and/or Kleptography[^287] and has many names such as Koalang[^288] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...