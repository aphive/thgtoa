...
Version 0.4.9 (draft), January 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^119] if your laptop supports it.
...
This technology is also widely used in CAPTCHAS[^234] services to verify that you are "human" and can be used to fingerprint a user.
...
Even if you're not looking at the camera, they can still figure out who you are[^141], make out your emotions[^142] and probably guess your political affiliation[^143].
...
-   If you absolutely want system wide plausible deniability[^190] despite the risks[^177].
...
If you can afford it, you might consider getting a Purism Librem laptop (<https://puri.sm>) or System76 laptops (<https://system76.com/>) while using Coreboot[^180] (where Intel IME is disabled from factory).
...
-   Enable BIOS/UEFI password and use a long passphrase[^181] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^182]. Keep it on if you intend to use Linux/Windows.
...
So what is Secure Boot[^183]? In short, it's a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically when the Operating Systems (or the Bootloader[^184]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^185] (with pictures).
...
This route will make extensive use of Virtual Machines[^186], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^192]'[^193] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
Cold Boot attacks[^196] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^197], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability. There have been some forensics studies[^198] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^199] and by the maintainer of Veracrypt[^200] so I wouldn't worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^202].

-   Windows Indexing keeping traces of the files present in your User folder by default[^203].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^204]'[^205].
...
-   Gatekeeper[^206] and XProtect keeping track of your download history in a local database
...
Forensics could[^199]'[^202] use all those leaks[^210] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Veracrypt[^207] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^208] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^209] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^210] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^211] Operations[^212]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^181] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^219] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
In this route, we will make extensive use of the free Oracle Virtualbox[^220] software. This is a virtualization software in which you can create Virtual Machines that emulate a computer running a specific OS (if you want to use something else like Xen, Qemu, KVM or VMWARE, feel free to do so but this part of the guide covers Virtualbox only for convenience).

You should be aware that Virtualbox is not the best virtualization software in terms of security and some of the reported issues[^221] have not be completely fixed to this date[^222] and if you're using Linux with a bit more technical skills, you should consider using KVM instead by following the guide available at Whonix here <https://www.whonix.org/wiki/KVM>
...
This route will use Virtualization and Whonix[^223] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later on you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^224] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
Also apply this fix from VirtualBox to mitigate Spectre[^226]/Meltdown[^227] vulnerabilities by running this command from the VirtualBox Program Directory:
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^230], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^232] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^233] authentication with the authenticator feature.
...
Captcha[^234] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^235] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^236]) and Cloudflare (hCaptcha[^237]). hCaptcha is used on 15% of the internet by their own metrics[^238].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^239]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^240].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^241].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^243].
...
Browser and Device[^244] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^245]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^246] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^247].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^248] and unfortunately sometimes required.
...
This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^249]. Unfortunately this method is now also sometimes being used on Facebook[^250] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^251])
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^245] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^252].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^253].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^254] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^257] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^258] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^259] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^260]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^261]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
This time you will need to submit an appeal using a form[^262], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using at least seven proxies[^263] that are not known by 4chan blocking system.
...
-   Use an Android Emulator (Youwave[^264], Bluestacks[^265] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
So the fact is that it's very very unlikely[^272]'[^273] for a forensic examiner to be able to recover data from a Trim enabled SSD but it's not completely impossible either[^274]'[^275]'[^276] if they're fast enough and have access to extensive equipment and skills.
...
Obviously for maximum security, Sanitize type options should be preferred. Unfortunately are no **free** easy (bootable with a menu) all-in-one tools available and you'll be left with either going with drive manufacturers provided tools, the free manual hdparm[^277] utility or going with a commercial tool such as PartedMagic[^278] or Blancco[^279].
...
This guide will therefore recommend the use of the free utility hdparm using a GParted[^280] boot USB.
...
Unfortunately Apple has removed the secure erase options from the trash bin even for HDD drives[^282]. So you're left with using other tools:
...
The best tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^283] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux quite easily. I never managed to make it work properly within Windows due various dependencies issues despite the provided instructions. It's however very straightforward to install and use on Linux.
...
Mat2 is also pre-installed on the Whonix Workstation VM[^284] and available on TAILS by default[^285].
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^210]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^202] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally I would just pick Normal Deletion + Trim (Trim itself should be enough[^214]). Secure Deletion with Trim[^211] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^286] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^287] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^288] and Secure Communication Obfuscation[^289] through Steganography[^290] and/or Kleptography[^291] and has many names such as Koalang[^292] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
As written earlier in this guide and as noted by Privacytools.io[^293], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   The Windows USN journal which stores plenty of information[^295].

-   The Windows System Resource Usage Monitor (SRUM)[^296].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^297].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^298]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
[^180]: Coreboot, <https://www.coreboot.org/>

[^181]: XKCD, Password Strength <https://xkcd.com/936/>

[^182]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^183]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^184]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^185]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^186]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^187]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^188]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^189]: Wikipedia, APNS <https://en.wikipedia.org/wiki/Apple_Push_Notification_service>

[^190]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^191]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^192]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^193]: GP Digital, World map of encryption laws and policies <https://www.gp-digital.org/world-map-of-encryption/>

[^194]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^195]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^196]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^197]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^198]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^199]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^200]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^201]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^202]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^203]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^204]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^205]: BlackBag, Windows 10 Jump List Forensics <https://www.blackbagtech.com/blog/windows-10-jump-list-forensics/>

[^206]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^207]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^208]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^209]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^210]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^211]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^212]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^213]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^214]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^215]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^216]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^217]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^218]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^219]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^220]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^221]: VirtualBox Ticket 17987 <https://www.virtualbox.org/ticket/17987>

[^222]: Whonix Documentation, Spectre Meltdown, <https://www.whonix.org/wiki/Spectre_Meltdown#VirtualBox>

[^223]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^224]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^225]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^226]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^227]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>
...
[^230]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^231]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^232]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^233]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^234]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^235]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^236]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^237]: hCaptcha <https://www.hcaptcha.com/>

[^238]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^239]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^240]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^241]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^242]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^243]: Privacy International,

    Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^244]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^245]: Developers Google Blog,
...
[^246]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^247]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^248]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^249]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^250]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^251]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^252]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^253]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^254]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^255]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^256]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^257]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^258]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^259]: Jumio, ID verification features <https://www.jumio.com/features/>

[^260]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^261]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^262]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^263]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^264]: Youwave, <https://youwave.com/>

[^265]: Bluestacks, <https://www.bluestacks.com/>

[^266]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^267]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^268]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^269]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^270]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^271]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^272]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^273]: ResearchGate, Live forensics method for acquisition on the Solid State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^274]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^275]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^276]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^277]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^278]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^279]: Blancco, Drive Eraser <https://www.blancco.com/products/drive-eraser/>

[^280]: Wikipedia, GParted <https://en.wikipedia.org/wiki/GParted>

[^281]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^282]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^283]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^284]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^285]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^286]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^287]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^288]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^289]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^290]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^291]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^292]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^293]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^294]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^295]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^296]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^297]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^298]: DBAN, <https://dban.org/>
