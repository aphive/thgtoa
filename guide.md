...
-   [Covering your tracks:]
    ...
    -   [Some measures against forensics:]
        ...
        -   [Linux:][3]
        -   [Windows:][4]
    -   [How to securely delete specific files/folders on your HDD/SDD drives:]
...
Version 0.4.5 (draft), January 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
Will this guide help you protect yourself from the NSA, the FSB, Mark Zuckerberg or the Mossad if they're out to find you? Probably not ... Mossad will be doing "Mossad things" [^4] and will probably find you no matter how hard you try to hide[^5].
...
A solution to this is to use encrypted DNS using DoH (DNS over HTTPS[^24]), DoT (DNS over TLS[^25]) with a private DNS server (this can be self-hosted locally with a solution like pi-hole[^26], remotely hosted with a solution like nextdns.io or using the solutions provider by your VPN provider or the Tor network). This should prevent your ISP or some middle-man from snooping on your requests ... except it might not.

Unfortunately the TLS protocol used in most HTTPS connections in most Browsers (Chrome/Brave among them) will leak the DNS again through SNI[^27] handshakes (this can be checked here at Cloudflare: <https://www.cloudflare.com/ssl/encrypted-sni/> ). **As of the writing of this guide**, **only Firefox based browsers supports eSNI (encrypted SNI**[^28] **soon to be renamed ECH**[^29] **and soon to be supported by Firefox too**[^30]**) which will encrypt everything end to end (in addition to using a secure private DNS over TLS/HTTPS) and will allow you to actually hide your DNS requests from a third party**[^31]**.** And this option is not enabled by default either so you'll have to enable it yourself:

In addition to limited browser support, only Web Services and CDNs[^32] behind Cloudflare CDN support eSNI at this stage (and will support ECH in the future[^33]).
...
Some countries like Russia[^34] and China[^35] will block eSNI handshakes at network level to allow snooping and prevent bypassing censorship. Meaning you won't be able to establish an HTTPS connection with a service if you don't allow them to see what it was.

Finally, even if you use a custom encrypted DNS server (DoS or DoT) with eSNI support, it might still not be enough as traffic analysis studies[^36] have shown it's still possible to reliably fingerprint and block unwanted requests. Only DNS over Tor was able to demonstrate efficient DNS Privacy in recent studies but even that can still be defeated by other means (see Your "Anonymized" Tor/VPN traffic:).

One could also decide to use a Tor Hidden DNS Service or ODoH (Oblivious DNS over HTTPS[^37]) to further increase privacy/anonymity but unfortunately, as far as I know, these services are only provided by Cloudflare as of this writing (<https://blog.cloudflare.com/welcome-hidden-resolver/>, https://blog.cloudflare.com/oblivious-dns/). I **[personally]{.ul}** think these are viable and reasonably secure technical options but there is also a moral choice if you want to use them (despite the risk posed by some researchers[^38]). Here is an illustration showing the current state of DNS privacy:
...
The IMEI (International Mobile Equipment Identity[^39]) and the IMSI (International Mobile Subscriber Identity[^40]) are unique numbers created by mobile phone manufacturers and mobile phone operators.

The IMEI is tied directly to the phone you're using. This number is known and tracked by the mobile phone operators and known by the manufacturers. Every time your phone connects to the mobile network, it will register the IMEI on the network along the IMSI (if a SIM card is inserted but that's not even needed). It's also used by many applications (Banking apps abusing the phone permission on Android for instance[^41]) and smartphone Operating Systems (Android/IOS) for identification of the device[^42]. It is possible but difficult (and not illegal in many jurisdiction[^43]) to change the IMEI on a phone but it's probably easier and cheaper to just find and buy some old (working) Burner phone for a few Euros (this guide is for Germany remember) at a flea market or at some random small shop.
...
-   The smartphone OS makers (Google/Apple for Android/IOs) also keep logs of IMEI/IMSI identifications tied to Google/Apple accounts and which user has been using them. They too can trace back the history of the phone and to which accounts it was tied in the past[^47].

-   Government agencies around the world interested in your phone number can and do use[^48] special devices called "IMSI catchers"[^49] like the Stingray[^50] or the Nyxcell[^51]. These devices are able to impersonate (to spoof) a cell phone Antenna and force a specific IMSI (from your phone) to connect to it to access the cell network. Once they do, they will be able to use various MITM[^52] (Man-In-The-Middle Attacks) that will allow them to:
...
For additional safety (if you absolutely have to take your smartphone along with you or if you want to store your Burner phone), you could consider the use of a faraday cage[^53] bag to store your devices. There are many such faraday "signal blocking" bags available for sale and some of these have been studied[^54] for their effectiveness. If you can't afford such bags, you can probably achieve a "decent result" with one or several sheets of aluminum foil (as shown in the previously linked study).

While there are some smartphones manufacturers like Purism with their Librem series[^55] who have your privacy in mind, they still do not allow IMEI randomization which I believe is a key anti-tracking feature that should be provided by such manufacturers. While this measure won't prevent IMSI tracking within the SIM card, it would at least allow you to keep the same "burner phone" and only switch SIM cards instead of having to switch both for privacy.

## Your Wi-Fi or Ethernet MAC address:

The MAC address[^56] is a unique identifier tied to your physical Network Interface (Wired Ethernet or WIFI) and could of course be used to track you if it's not randomized. As it was the case with the IMEI, manufacturers of computers and network cards usually keep logs of their sales (usually including things like: Serial number, IMEI, Mac Addresses, ...) and it's possible again for them to track where and when the computer with the MAC address in question was sold and to whom. Even if you bought it with cash in a supermarket, the supermarket might still have CCTV (or a CCTV just outside that shop) and again the time/date of sale could be used to find out who was there using the Mobile Provider antenna logs at that time (IMEI/IMSI).
...
Operating systems have protections in place to randomize those addresses but are still subject to vulnerabilities[^57].
...
-   IOS/Apple:
    ...
    -   Apple does claim[^58] that they anonymize this data using differential privacy[^59] but you'll have to trust them on that.
...
But that's not all those WiFis access points can do. Recently developed techs could even allow someone to track your movements accurately just based on radio interferences. What this means is that it's possible to track your movement inside a room/building based on the radio signals passing through. This might seem like a tinfoil hat conspiracy theory claim but here are the references[^62] with demonstrations showing this tech in action: <http://rfpose.csail.mit.edu/> and the video here: <https://www.youtube.com/watch?v=HgDdaMy8KNE>
...
Your metadata is all the information about your activities without the actual content of those activities. For instance it's like knowing you had a call from an oncologist before then calling your family and friends successively. You don't know what was said during the conversation but you can guess what it was just from the "metadata"[^63].

This metadata will also often include your location that is being harvested by Smartphones, Operating Systems (Android[^64]/IOS), Browsers, Apps, Websites. Odds are there are several companies knowing exactly where you are at any time[^65] because of your smartphone[^66].

This location data has been used in many judicial cases[^67] already as part of "geofence warrants" [^68] that allows law enforcement to ask companies (such as Google/Apple) a list of all devices present at a certain location at a certain time.
...
The question is: Is there someone somewhere that would possibly have both pieces of information available[^69] for correlation in a convenient database?

Have you heard of Edward Snowden[^70]? Now is the time to google him and read his book[^71]. I recommend reading about XKEYSCORE[^72]'[^73], MUSCULAR[^74] and PRISM[^75].

"We kill people based on Metadata"[^76]
...
All of this data is very likely being transmitted, processed and stored indefinitely (unencrypted[^77]) by various third parties[^78].
...
-   Your Fitness Devices and Apps? (Strava[^79]'[^80], Fitbit[^81], Garmin, Polar[^82], ...)[^83]

-   Your Smart Speaker? (Amazon Alexa[^84], Google Echo, Apple Homepod ...).
...
-   IPhones and IPads (IOS 13 and above)[^85]'[^86].

-   Samsung Phones (Android 10 and above)[^87].

-   MacBooks (MacOS 10.15 and above)[^88].

Such devices will continue to broadcast identity information to nearby devices even when offline using Bluetooth Low-Energy[^89]. They don't have access to the devices directly (which are not connected to the internet) but instead use BLE to find them through other nearby devices[^90].
...
RFID stands for Radio-frequency identification[^91], it's the technology used for instance for contactless payments and various identification systems. Of course your smartphone is among those devices and has RFID contactless payment capabilities through NFC[^92]. As with everything else, such capabilities can be used for tracking by various actors.
...
The only way to mitigate this problem is to have no RFID tags on you or to shield them again using a type of faraday cage. You could also use specialized wallets/pouches that specifically block RFID communications. Many of those are now made by well-known brands such as Samsonite[^93].

## Your Files Properties/Metadata:

This can be obvious to many but not to all. Most files have metadata attached to them. A good example are pictures which store EXIF[^94] information which can contain a lot of information such as GPS coordinates, which camera/phone model took it and when it was taken precisely. While this information might not directly give out who you are, it could tell exactly where you were at a certain moment which could allow others to use different sources to find you (CCTV or other footage taken at the same place at the same time during a protest for instance). It's important that you verify any file you would put on those platforms for any properties that might contain any information that might lead back to you.

Here is an example of EXIF data that could be on a picture:![][10]
...
**Even if you publish a simple text file, you should always double or triple check it for any information leakage before publishing.**
...
Tor and VPNs are not silver bullets. Many advanced techniques have been developed and studied to de-anonymize encrypted traffic over the years[^95]. Most of those techniques are Correlation attacks that will correlate your network traffic in one way or another to logs or datasets. Here are some classic examples:

-   Correlation Fingerprinting Attack: As illustrated (simplified) below, this attack will fingerprint[^96] your encrypted traffic (like the websites you visited) just based on the analysis of your encrypted traffic (without decrypting it). It's able to do so with a whopping 96% success rate. Such fingerprinting can be used by an adversary that has access to your source network to figure out some of your encrypted activity (such as which websites you visited).

![][13]

-   Correlation Timing Attacks: As illustrated (simplified) below, an adversary that has access to network connection logs (IP or DNS for instance, remember that most VPN servers and most Tor nodes are known and publicly listed) at the source and at the destination could correlate the timings to de-anonymize you without requiring any access to the Tor or VPN network in between. A real use case of this technique was done by the FBI in 2013 to de-anonymize[^97] a bomb threat hoax at Harvard University.
...
Be aware again that this might not be enough against a motivated global adversary[^98] with wide access to global mass surveillance (remember XKEYSCORE, MUSCULAR and PRISM). Such adversary might have access to logs no matter where you are and could use those to de-anonymize you.
...
Lastly, do remember that using Tor in itself can already be considered a suspicious activity[^99] and its use could be considered malicious by some[^100].
...
Contrary to popular belief, Crypto transactions (such as Bitcoin and Ethereum) are not anonymous[^101]. Most crypto currencies can be tracked accurately through various methods[^102].

The main issue is not setting up a random Crypto wallet to receive some currency behind a VPN/Tor address (at this point, the wallet is anonymous). The issue is mainly when you want to convert Fiat money (Euros, Dollars ...) to Crypto and then when you want to cash in your Crypto. You'll have few realistic options but to transfer those to an exchange (such as Coinbase/Kraken/Bitstamp/Binance). Those exchanges have known wallet addresses and will keep detailed logs (due to KYC[^103] financial regulations) and can then trace back those crypto transactions to you using the financial system.

There are some crypto currencies with privacy in mind like Monero but even those can be de-anonymized to some extent[^104][^105].

Even if you use Mixers or Tumblers (services that specialize in anonymizing crypto currencies by "mixing them"), keep in mind this is only obfuscation and not actual anonymity[^106].

## Exploits in your apps and services:

So you're using Tor Browser or Brave Browser with a Tor Tab. You could be using those over a VPN for added security. But you should keep in mind that there are exploits[^107] (hacks) that could be known by an adversary (but unknown to the App/Browser provider). Such exploits could be used to compromise your system and reveal details to de-anonymize you such as your IP address or other details.

A real use case of this technique was the Freedom Hosting[^108] case in 2013 where the FBI inserted malware[^109] using a Firefox browser exploit on a Tor website. This exploit allowed them to reveal details of some users. More recently, there was the notable SolarWinds[^110] hack that breached several US government institutions by inserting malware into an official software update server.
...
To reflect these recommendations, this guide will therefore later guide you in the use of Virtualization so that even if your Browser/Apps get compromised by a skilled adversary, that adversary will find himself stuck in a sandbox[^111] without being able to access identifying information or compromise your system.

## Your CPU:

All modern CPUs[^112] are now integrating hidden management platforms such as the now infamous Intel Management Engine[^113] and the AMD Platform Security Processor[^114].
...
These have already been affected by several security vulnerabilities in the past[^115] that allowed malware to gain control of target systems. These are also accused by many privacy actors including the EFF and Libreboot of being a backdoor into any system[^116].

There are some not so easy ways[^117] to disable the Intel IME on some CPUs and you should do so if you can. For some AMD laptops, you can disable it within the BIOS settings by disabling PSP.

If you're feeling a bit more adventurous, you could install your own BIOS using Libreboot[^118].
...
These have been used since at least since 2008 using an attack called "Jasager"[^119] and can be done by anyone using self-built tools or using commercially available devices such as Wi-Fi Pineapple[^120].
...
These devices can fit in a small bag and can take over the Wi-Fi environment of any place within their range. For instance a Bar/Restaurant/Café/Hotel Lobby. These devices can force Wi-Fi clients to disconnect from their current Wi-Fi (using de-authentication, disassociation attacks[^121]) while spoofing the normal Wi-Fi networks at the same location. They will continue to perform this attack until your computer or yourself decides to try to connect to the rogue AP.

These devices can then mimic a captive portal[^122] with the exact same layout as the Wi-Fi you're trying to access (for instance an Airport Wi-Fi registration portal). Or they could just give you open access internet that they'll themselves get from the same place.
...
There are readily available commercial and cheap "badUSB" [^123]devices that can take deploy malware, log your typing, geolocate you, listen to you or gain control of your laptop just by plugging them in. Here are some examples that you can already buy yourself.
...
Such devices can be implanted anywhere (charging cable, mouse, keyboard, USB key ...) by an adversary and can be used to track you or compromise your computer or smartphone. The most notable example of such attacks is probably Stuxnet[^124] in 2005.

While you could inspect an USB key physically, Scan it with various utilities, check the various components to see if they're genuine, you will most likely never be able to discover complex malware embedded in genuine parts of a genuine USB key by a skilled adversary without advanced forensics equipment[^125].
...
There are specialized commercial forensics solutions available (Magnet Axiom[^126], Cellebrite Cloud[^127]) that will help an adversary analyze your cloud data with ease.
...
The digital fingerprint is the way you write[^128], the way you behave[^129]. The way you click. The way you browse. The fonts you use on your browser[^130]. Fingerprinting is being used to guess who someone is by the way that user is behaving. You might be using specific pedantic words or making specific spelling mistakes that could give you away using a simple Google search for similar features because you typed in a similar way on some Reddit post 5 years ago using a not so anonymous Reddit account.
...
This technology is also widely used in CAPTCHAS[^222] services to verify that you are "human" and can be used to fingerprint a user.
...
A real use and well-documented case of this was the arrest of the hacker Jeremy Hammond[^131] who shared over time several details about his past and was later discovered.

There are also a few cases involving OSINT at Bellingcat[^132].Have a look at their very informative toolkit here: <https://bit.ly/bcattools>
...
Your Browser and Device Fingerprints[^232] are set of properties/capabilities of your System/Browser. These are used on most websites for invisible user tracking but also to adapt the website user experience depending on their browser. For instance websites will be able to provide a "mobile experience" if you're using a mobile browser or propose a specific language/geographic version depending on your fingerprint. Most of those techniques work with recent Browsers like Chromium[^133] based browsers (such as Chrome) or Firefox[^134] unless taking special measures.
...
Companies like Facebook have used advanced face recognition for years[^135]'[^136] and have been using other means (Satellite imagery) to create maps of "people" around the world[^137].
...
Even if you're not looking at the camera, they can still figure out who you are[^140] and even make out your emotions[^141].
...
-   Even if you never made a profile on those platforms, you still have one without even knowing it[^142]'[^143]'[^144]'[^145]'[^146].
...
Governments already know who you are because they have your ID/Passport/Driving License pictures and often added biometrics (Fingerprints) in their database. Those same governments are integrating those technologies (often provided by private companies such as the Israeli AnyVision[^147]) in their CCTV networks to look for "persons of interest"[^148]. And some heavily surveilled states like China have implemented widespread use of Facial Recognition for various purposes[^149] including possibly identifying ethnic minorities[^150]. A simple face recognition error by some algorithm can ruin your life[^151].
...
We can safely imagine a near future where you won't be able to create accounts or sign-in anywhere without providing unique biometrics (A good time to re-watch Gattaca[^152], Person of Interest[^153] and Minority Report[^154]). And you can safely imagine how useful these large biometrics databases could be to some interested third parties.

At this time, there are a few steps[^155] you can use to mitigate (and only mitigate) face recognition when conducting sensitive activities where CCTV might be present:

-   Wear a facemask as they have been proven to defeat some face recognition technologies[^156].
...
Phishing[^157] is a type of attack where an adversary could try to extract information from you by pretending to be something/someone else.

A typical case is an adversary using a man-in-the-middle[^52] attack or a falsified e-mail/call to ask your credential for a service. This can be your e-mail or your financial services for example.
...
Forensics techniques are now very advanced and can reveal a staggering amount information from your devices even if they're encrypted[^158]. These techniques are widely used by law enforcement all over the world and should be considered.
...
And last but not least here is this very instructive detailed paper on the current state of IOS/Android security from the John Hopkins University: https://securephones.io/main.html[^159]
...
Any of those providers can be forced to silently (without your knowing (using for example a court order with a gag order[^160] or a national security letter[^161]) log your activity to de-anonymize you. There have been several recent examples of those:

-   2020, The Germany based mail provider Tutanota was forced to implement a backdoor to save unencrypted copies of the e-mails of one user[^162].

-   2017, PureVPN was forced to disclose information of one user to the FBI[^163].

-   2014, EarthVPN user was arrested based on logs provider to the Dutch Police[^164].

-   2014, HideMyAss user was de-anonymized and logs were provider to the FBI[^165].

-   2013, Secure E-Mail provider Lavabit shuts down after fighting a secret gag order[^166].

Some providers have implemented the use of a Warrant Canary[^167] that would allow their users to find out if they have been compromised by such orders but this has not been tested yet as far as I know.

Last but not least, it's now well known that some companies might be sponsored front-ends for some state adversaries (see the Crypto AG story[^168] and Omnisec story[^169]).
...
There are many advanced techniques that can be used by skilled adversaries[^170] to bypass your security measures provided they already know where your devices are. Many of those techniques are detailed here <https://cyber.bgu.ac.il/advanced-cyber/airgap> (Air-Gap Research Page, Cyber-Security Research Center, Ben-Gurion University of the Negev, Israel) and include:
...
-   **Attacks that require no malware:**
    ...
    -   Observing a light bulb from a distance to listen to the sound in the room[^171] **without any malware**: Demonstration: <https://www.youtube.com/watch?v=t32QvpfOHqw>
...
-   If you absolutely want system wide plausible deniability[^183] despite the risks[^172].
...
TAILS[^174] stands for **The Amnesic Incognito Live System.** . It's a bootable Live Operating System running from a USB key that is designed for leaving no traces and forcing all connections through the Tor network.
...
Note: If you can afford it, you might consider getting a Purism Librem laptop: <https://puri.sm>. If you cannot I would still recommend getting an "Enterprise type laptop" such as a Lenovo ThinkPad over consumer ones such as Asus/MSI/Gigabyte because they just have better hardware security options available generally speaking.
...
-   Enable BIOS/UEFI password and use a long passphrase[^175] instead of a password if you can and make sure this password is required for:
...
-   Disable Secure Boot if you intend to use QubesOS as they do not support it out of the box[^176]. Keep it on if you intend to use Linux/Windows.
...
So what is Secure Boot[^177]? In short, it's a UEFI security feature designed to prevent your computer from booting an operating system from which the bootloader was not signed by specific keys stored in the UEFI firmware of your laptop.

Basically when the Operating Systems (or the Bootloader[^178]) supports it, you can store the keys of your bootloader in your UEFI firmware and this will prevent booting up any unauthorized Operating System (such as a live OS USB or anything similar).
...
Here is a good cheap method to make your laptop tamper proof using Nail Polish (with glitter) <https://mullvad.net/en/help/how-tamper-protect-laptop/> [^179] (with pictures).
...
This route will make extensive use of Virtual Machines[^180], they'll require a host OS to run the Virtualization software. You have 3 recommended choices in this part of the guide:
...
This guide will not really recommend using MacOS Big Sur as the host OS due to the "unblockable" telemetry[^181]'[^182] issues. In addition, changes are high that your Mac is or has been tied to an Apple account (at the time or purchase or after signing-in) and therefore its unique hardware identifiers could lead back to you in case of hardware identifiers leak.

Linux is not necessarily the best choice for privacy depending on your threat model. This is because using Windows will allow us to use Plausible Deniability[^183] at the OS level.

Windows is also unfortunately a privacy nightmare[^184] but is the only option for using OS wide plausible deniability. Windows telemetry and telemetry blocking is also widely documented which should mitigate many issues.
...
A soft lawful adversary could ask for your encrypted laptop password. At first you could refuse to give out any password (using your "right to remain silent", "right not to incriminate yourself") but some countries are implementing laws[^185] to exempt this from such rights (because terrorists and "think of the children"). In that case you might have to reveal the password or maybe face jail time in contempt of court. This is where plausible deniability will comes into play.
...
Cold Boot attacks[^188] are trickier than the Evil Maid Attack but can be part of an Evil Maid attack as it requires an adversary to come into possession of your laptop while you're actively using your device or shortly afterward.

The idea is rather simple, as shown in this video[^189], an adversary could theoretically quickly boot your device on a special USB key that would copy the content of the RAM (the memory) of the device after you shut it down. If the USB ports are disabled or if they feel like they need more time, they could open it and "cool down" the memory using a spray or other chemicals (liquid nitrogen for instance) preventing the memory decaying. They could then be able to copy its content for analysis. This memory dump could contain the key to decrypt your device. We will later apply a few principles to mitigate these.

In the case of Plausible Deniability. There have been some forensics studies[^190] about technically proving the presence of the hidden data with a simple forensic examination (without a Cold Boot/Evil Maid Attack) but these have been contested by other studies[^191] and by the maintainer of Veracrypt[^192] so I wouldn't worry too much about those yet.
...
-   Windows ShellBags that are stored within the Windows Registry silently storing various histories of accessed volumes/files/folders[^194].

-   Windows Indexing keeping traces of the files present in your User folder by default[^195].

-   Recent lists (aka Jump Lists) in Windows and various apps keeping traces of recently accessed documents[^196]'[^197].
...
-   Gatekeeper[^198] and XProtect keeping track of your download history in a local database
...
Forensics could[^191]'[^194] use all those leaks[^202] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
Veracrypt[^199] is the software I will recommend for full disk encryption, file encryption and plausible deniability. It is a fork of the well-known but deprecated and unmaintained TrueCrypt. It can be used for
...
It is to my knowledge the only (convenient and usable by anyone) free, open-source and openly audited[^200] encryption software that also provides plausible deniability for general use and it works with Windows Home Edition.
...
-   Encrypt the memory with a Veracrypt option[^201] (settings \> performance/driver options \> encrypt RAM) at a cost of 5-15% performance. This will also disable hibernation (which doesn't clear the key when hibernating) and instead encrypt the memory altogether to mitigate some cold-boot attacks.
...
-   Enable the Veracrypt option to mount volumes as removable volumes (Settings \> Preferences \> Mount volume as removable media). This will prevent Windows from writing some logs about your mounts in the Event logs[^202] and prevent some local data leaks.
...
-   Route C: Plausible Deniability Encryption of your Operating system (you'll have a "hidden OS" and a "decoy OS" running on the laptop):
    ...
    -   Cons:
        ...
        -   No use with an SSD drive due to requirement of disabling Trim[^203] Operations[^204]. This will severely degrade the performance/health of your SSD drive over time.
...
-   Use a strong passphrase[^175] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
-   To rescue disk[^211] or not rescue disk, well that's up to you. I recommend making one (just in case), just make sure to store it outside your encrypted drive (USB key for instance, or wait and see the end of this guide for guidance on safe backups). This rescue disk will not store your passphrase and you'll still need it to use it.
...
This route will use Virtualization and Whonix[^213] as part of the anonymization process. Whonix is a Linux distribution composed of two Virtual Machines:
...
Later on you will create and run several Virtual Machines within Virtualbox for your sensitive activities. Virtualbox provides a feature called "Snapshots"[^214] that allow for saving the state of a VM at any point in time. If for any reason later you want to go back to that state, you can restore that snapshot at any moment.
...
You will need something to store your data (logins/passwords, identities and TOTP[^216] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^217] authentication with the authenticator feature.
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^218], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^220] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^221] authentication with the authenticator feature.
...
Captcha[^222] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^223] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^224]) and Cloudflare (hCaptcha[^225]). hCaptcha is used on 15% of the internet by their own metrics[^226].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^227]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^228].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^229].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^231].
...
Browser and Device[^232] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^233]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^234] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^235].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^236] and unfortunately sometimes required.
...
Some platforms/apps will require you to take a live picture of yourself either doing something (a wink, holding an arm up ...) or showing a custom piece of information (a hand written text, a passport or ID) within the picture. Sometimes the platform/app will require several pictures to increase their certainty.![][27]

This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^237]. Unfortunately this method is now also sometimes being used on Facebook[^238] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^239])
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^233] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^240].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^241].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^242] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^245] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^246] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^247] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^248]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^249]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
This time you will need to submit an appeal using a form[^250], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using at least seven proxies[^251] that are not known by 4chan blocking system.
...
-   Use an Android Emulator (Youwave[^252], Bluestacks[^253] or if you have a powerful PC, Android Studio) within the Windows VM and run the App through your multi-layer of Tor/VPN. Drawback is that such emulators are usually quite resource hungry and will slow down your VM and use more battery. Here is also an (outdated) guide on this matter: <https://www.bellingcat.com/resources/how-tos/2018/08/23/creating-android-open-source-research-device-pc/>
...
**Trim is your ally and should always be enabled when using an SSD drive and should offer sufficient protection**. And this is also the reason you should not use Veracrypt Plausible deniability on a Trim enabled SSD as this feature is incompatible with Trim[^258]. Here is an illustration from Wikipedia showing how it works on an SSD drive:
...
So the fact is that it's very very unlikely[^260]'[^261] for a forensic examiner to be able to recover data from a Trim enabled SSD but it's not completely impossible either[^262]'[^263]'[^264] if they're fast enough and have access to extensive equipment and skills.
...
Obviously for maximum security, Sanitize type options should be preferred. Unfortunately are no **free** easy (bootable with a menu) all-in-one tools available and you'll be left with either going with drive manufacturers provided tools, the free manual hdparm[^265] utility or going with a commercial tool such as PartedMagic[^266] or Blancco[^267].
...
This guide will therefore recommend the use of the free utility hdparm using a GParted[^268] boot USB.
...
-   Option A: See Appendix E: Using Gparted to Securely Wipe an SSD drive.

-   Option B: Wipe your disk and re-install Linux with a new full disk encryption to overwrite all sectors with new encrypted data.
...
-   Option B: Wipe your disk and re-install Windows before performing a new full disk encryption (using Veracrypt or Bitlocker) to overwrite all sectors with new encrypted data.

#### HDD:

There are several utilities that are recommend (like the old unmaintained DBAN[^269]) for this but personally, I will recommend the use of ShredOS.
...
In most cases, if your disk was encrypted with Filevault and you just perform a normal erase, it should be "enough". You should then re-install MacOS again and re-encrypt it with Filevault again after re-installing. This should perform a crypro erase by overwriting your previous install and encryption.
...
**I'm not aware of any reliable method to selectively securely delete single files on SSD drives unless overwriting ALL the free space (which might reduce the lifespan of your SSD) after Deletion + Trim of these files. Without doing that, you'll have to trust the SSD Trim operation for this which in my opinion is enough. It's reasonable.**

In addition, most of these measures here should not be needed since your whole drive should be encrypted and therefore your data should not be accessible for forensic analysis anyway even through SSD/HDD examination. So these are just "bonus measures" for weak/unskilled adversaries but the best thing to do is to wipe the whole drive as explained earlier.
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^202]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^194] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
After cleaning those previous traces, you should also use third party utilities than can be used to clean various traces and most importantly free space. In this guide I will recommend two of them:
...
-   The Windows USN journal which stores plenty of information[^270].

-   The Windows System Resource Usage Monitor (SRUM)[^271].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^272].
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally I would just pick Normal Deletion + Trim (Trim itself should be enough[^206]). Secure Deletion with Trim[^203] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
With an SSD drive however, again everything becomes complicated because you're never sure anything is really deleted due to reliance on the Trim operation of the drive. An adversary that has the decryption key of your SDD (whether it's LUKS, Filevault 2, Veracrypt or Bitlocker) could unlock your drive and then attempt recovery using classic recovery utilities[^273] it the data wasn't trimmed properly. But this is again highly unlikely.

Therefore again the only way to be 100% sure a file is securely deleted from your unlocked encrypted SDD is to again overwrite all the free space after deletion of the files in question. But I think this is highly overkill.

If you're using an HDD drive and installed one of the above utilities, you can just right click a file and use the contextual options:

-   Delete without a trace

-   Shred with BleachBit

These should work without issues on HDD drives but, if you are running an SSD drive, you have no choice but to either Delete and trust the Trim operation of the SSD drive to remove it or perform a full overwriting of the free disk space after deletion of the file/folder using PrivaZer or BleachBit after deleting the files.

I think you can and should trust the Trim operation in most cases and for some safety, you could still trigger an additional forced Trim using the above Windows Optimize tool that will force a trim. If you're really paranoid, you could fill up all the free space in addition to these. And if you want absolute certainty, you have to refer to the previous chapter" How to securely wipe your whole laptop if you want to erase everything:"
...
DuckDuckGo does not store cached version of pages[^274] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^275] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^276] and Secure Communication Obfuscation[^277] through Steganography[^278] and/or Kleptography[^279] and has many names such as Koalang[^280] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
-   Doxx "yourself" (your identities) frequently by looking for them yourself online using various search engines to monitor your online identities.
...
<!-- -->
```
-   Securely erase the laptop hard drive "

```{=html}
<!-- -->
```
-   " and then ideally proceed to physically destroy the HDD/Laptop and trash it somewhere.
...
-   Contact a lawyer if possible and hope for the best.
...
As written earlier in this guide and as noted by Privacytools.io[^281], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
[^29]: Wikipedia, ECH, <https://en.wikipedia.org/wiki/Server_Name_Indication#Encrypted_Client_Hello>

[^30]: Mozilla Blog, Encrypted Client Hello: the future of ESNI in Firefox <https://blog.mozilla.org/security/2021/01/07/encrypted-client-hello-the-future-of-esni-in-firefox/>

[^31]: Usenix.org, On the Importance of Encrypted-SNI (ESNI) to Censorship Circumvention <https://www.usenix.org/system/files/foci19-paper_chai_0.pdf>

[^32]: Wikipedia, CDN <https://en.wikipedia.org/wiki/Content_delivery_network>

[^33]: Cloudflare, Good-bye ESNI, hello ECH! <https://blog.cloudflare.com/encrypted-client-hello/>

[^34]: ZDNET, Russia wants to ban the use of secure protocols such as TLS 1.3, DoH, DoT, ESNI <https://www.zdnet.com/article/russia-wants-to-ban-the-use-of-secure-protocols-such-as-tls-1-3-doh-dot-esni/>

[^35]: ZDNET, China is now blocking all encrypted HTTPS traffic that uses TLS 1.3 and ESNI <https://www.zdnet.com/article/china-is-now-blocking-all-encrypted-https-traffic-using-tls-1-3-and-esni/>

[^36]: KUL, Encrypted DNS=⇒Privacy? A Traffic Analysis Perspective <https://www.esat.kuleuven.be/cosic/publications/article-3153.pdf>

[^37]: ResearhGate, Oblivious DNS: Practical Privacy for DNS Queries <https://www.researchgate.net/publication/332893422_Oblivious_DNS_Practical_Privacy_for_DNS_Queries>

[^38]: Nymity.ch, The Effect of DNS on Tor's Anonymity <https://nymity.ch/tor-dns/>

[^39]: Wikipedia, IMEI <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity>

[^40]: Wikipedia, IMSI <https://en.wikipedia.org/wiki/International_mobile_subscriber_identity>

[^41]: Android Documentation, Device Identifiers <https://source.android.com/devices/tech/config/device-identifiers>

[^42]: Google Privacy Policy, Look for IMEI <https://policies.google.com/privacy/embedded?hl=en-US>

[^43]: Wikipedia, IMEI and the Law <https://en.wikipedia.org/wiki/International_Mobile_Equipment_Identity#IMEI_and_the_law>
...
[^46]: Wikipedia, CCTV <https://en.wikipedia.org/wiki/Closed-circuit_television>

[^47]: Apple, Transparency Report, Device Requests <https://www.apple.com/legal/transparency/device-requests.html>

[^48]: The Intercept, How Cops Can Secretly Track Your Phone <https://theintercept.com/2020/07/31/protests-surveillance-stingrays-dirtboxes-phone-tracking/>

[^49]: Wikipedia, IMSI Catcher <https://en.wikipedia.org/wiki/IMSI-catcher>

[^50]: Wikipedia, Stingray <https://en.wikipedia.org/wiki/Stingray_phone_tracker>

[^51]: Gizmodo, Cops Turn to Canadian Phone-Tracking Firm After Infamous \'Stingrays\' Become \'Obsolete\' <https://gizmodo.com/american-cops-turns-to-canadian-phone-tracking-firm-aft-1845442778>

[^52]: Wikipedia, MITM <https://en.wikipedia.org/wiki/Man-in-the-middle_attack>

[^53]: Wikipedia, Faraday Cage, <https://en.wikipedia.org/wiki/Faraday_cage>

[^54]: Edith Cowan University, A forensic examination of several mobile device Faraday bags & materials to test their effectiveness materials to test their effectiveness <https://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1165&context=adf>

[^55]: Purism, Librem 5 <https://shop.puri.sm/shop/librem-5/>

[^56]: Wikipedia, MAC Address <https://en.wikipedia.org/wiki/MAC_address>

[^57]: ResearchGate, Tracking Anonymized Bluetooth Devices <https://www.researchgate.net/publication/334590931_Tracking_Anonymized_Bluetooth_Devices/fulltext/5d3308db92851cd04675a469/Tracking-Anonymized-Bluetooth-Devices.pdf>

[^58]: Apple, Differential Privacy White Paper <https://www.apple.com/privacy/docs/Differential_Privacy_Overview.pdf>

[^59]: Wikipedia, Differential Privacy <https://en.wikipedia.org/wiki/Differential_privacy>

[^60]: Google Android Help, Android Location Services <https://support.google.com/accounts/answer/3467281?hl=en>

[^61]: Apple Support, Location Services and Privacy <https://support.apple.com/en-us/HT207056>

[^62]: State University of New York, Towards 3D Human Pose Construction Using Wi-Fi <https://cse.buffalo.edu/~lusu/papers/MobiCom2020.pdf>

[^63]: Using Metadata to find Paul Revere (<https://kieranhealy.org/blog/archives/2013/06/09/using-metadata-to-find-paul-revere/>)

[^64]: Wikipedia, Google SensorVault, <https://en.wikipedia.org/wiki/Sensorvault>

[^65]: NRKBeta, My Phone Was Spying on Me, so I Tracked Down the Surveillants <https://nrkbeta.no/2020/12/03/my-phone-was-spying-on-me-so-i-tracked-down-the-surveillants/>

[^66]: New York Times <https://www.nytimes.com/interactive/2019/12/19/opinion/location-tracking-cell-phone.html>

[^67]: Sophos, Google data puts innocent man at the scene of a crime <https://nakedsecurity.sophos.com/2020/03/10/google-data-puts-innocent-man-at-the-scene-of-a-crime/>

[^68]: Wikipedia, Geofence Warrant <https://en.wikipedia.org/wiki/Geo-fence_warrant>

[^69]: Wikipedia, Room 641A <https://en.wikipedia.org/wiki/Room_641A>

[^70]: Wikipedia, Edward Snowden <https://en.wikipedia.org/wiki/Edward_Snowden>

[^71]: Wikipedia, Permanent Record <https://en.wikipedia.org/wiki/Permanent_Record_(autobiography)>

[^72]: Wikipedia, XKEYSCORE <https://en.wikipedia.org/wiki/XKeyscore>

[^73]: ElectroSpaces, Danish military intelligence uses XKEYSCORE to tap cables in cooperation with the NSA <https://www.electrospaces.net/2020/10/danish-military-intelligence-uses.html>

[^74]: Wikipedia, MUSCULAR <https://en.m.wikipedia.org/wiki/MUSCULAR_(surveillance_program)>

[^75]: Wikipedia, PRISM <https://en.wikipedia.org/wiki/PRISM_(surveillance_program)>

[^76]: Justsecurity, General Hayden <https://www.justsecurity.org/10318/video-clip-director-nsa-cia-we-kill-people-based-metadata/>

[^77]: Reuters, Exclusive: Apple dropped plan for encrypting backups after FBI complained -- sources <https://www.reuters.com/article/us-apple-fbi-icloud-exclusive-idUSKBN1ZK1CT>

[^78]: ZDnet, I asked Apple for all my data. Here\'s what was sent back <https://www.zdnet.com/article/apple-data-collection-stored-request/>

[^79]: Wired, The Strava Heat Map and the End of Secrets <https://www.wired.com/story/strava-heat-map-military-bases-fitness-trackers-privacy/>

[^80]: Bellingcat, How to Use and Interpret Data from Strava\'s Activity Map <https://www.bellingcat.com/resources/how-tos/2018/01/29/strava-interpretation-guide/>

[^81]: The Guardian, Fitness tracking app Strava gives away location of secret US army bases <https://www.theguardian.com/world/2018/jan/28/fitness-tracking-app-gives-away-location-of-secret-us-army-bases>

[^82]: Telegraph, Running app reveals locations of secret service agents in MI6 and GCHQ <https://www.telegraph.co.uk/technology/2018/07/08/running-app-exposes-mi6-gchq-workers-whereabouts/>

[^83]: De Correspondent, Here's how we found the names and addresses of soldiers and secret agents using a simple fitness app <https://decorrespondent.nl/8481/heres-how-we-found-the-names-and-addresses-of-soldiers-and-secret-agents-using-a-simple-fitness-app/412999257-6756ba27>

[^84]: Washington Post, Alexa has been eavesdropping on you this whole time <https://www.washingtonpost.com/technology/2019/05/06/alexa-has-been-eavesdropping-you-this-whole-time/?utm_term=.8514f3a17b1c&itid=lk_interstitial_manual_59>

[^85]: CryptoEngineering, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^86]: Apple Support <https://support.apple.com/en-us/HT210515>

[^87]: XDA, Samsung's Find My Mobile app can locate Galaxy devices even when they're offline <https://www.xda-developers.com/samsung-find-my-mobile-app-locate-galaxy-devices-offline/>

[^88]: Apple Support, If your Mac is lost or stolen <https://support.apple.com/en-us/HT204756>

[^89]: Wikipedia, BLE <https://en.wikipedia.org/wiki/Bluetooth_Low_Energy>

[^90]: Cryptography Engineering Blog, How does Apple (privately) find your offline devices? <https://blog.cryptographyengineering.com/2019/06/05/how-does-apple-privately-find-your-offline-devices/>

[^91]: Wikipedia, RFID <https://en.wikipedia.org/wiki/Radio-frequency_identification>

[^92]: Wikipedia, NFC <https://en.wikipedia.org/wiki/Near-field_communication>

[^93]: Samsonite Online Shop, RFID accessories, <https://shop.samsonite.com/accessories/rfid-accessories/>

[^94]: Bellingcat, Joseph Mifsud: Rush for the EXIF <https://www.bellingcat.com/news/americas/2018/10/26/joseph-mifsud-rush-exif/>

[^95]: HackerFactor Blog, Deanonymizing Tor Circuits <https://www.hackerfactor.com/blog/index.php?/archives/868-Deanonymizing-Tor-Circuits.html>

[^96]: KU Leuven, Website Fingerprinting through Deep Learning <https://distrinet.cs.kuleuven.be/software/tor-wf-dl/>

[^97]: DailyDot, How Tor helped catch the Harvard bomb threat suspect <https://www.dailydot.com/unclick/tor-harvard-bomb-suspect/>

[^98]: ArsTechnica, How the NSA can break trillions of encrypted Web and VPN connections <https://arstechnica.com/information-technology/2015/10/how-the-nsa-can-break-trillions-of-encrypted-web-and-vpn-connections/>

[^99]: ArsTechnica, Does Tor provide more benefit or harm? New paper says it depends <https://arstechnica.com/gadgets/2020/11/does-tor-provide-more-benefit-or-harm-new-paper-says-it-depends/>

[^100]: ResearchGate, The potential harms of the Tor anonymity network cluster disproportionately in free countries <https://www.pnas.org/content/early/2020/11/24/2011893117>

[^101]: arXiv, An Analysis of Anonymity in the Bitcoin System <https://arxiv.org/abs/1107.4524>
...
[^103]: Wikipedia, KYC <https://en.wikipedia.org/wiki/Know_your_customer>

[^104]: YouTube, Breaking Monero <https://www.youtube.com/watch?v=WOyC6OB6ezA&list=PLsSYUeVwrHBnAUre2G_LYDsdo-tD0ov-y>

[^105]: Monero, Monero vs Princeton Researchers, <https://monero.org/monero-vs-princeton-researchers/>

[^106]: ArXiv, Tracking Mixed Bitcoins, <https://arxiv.org/abs/2009.14007>

[^107]: Wikipedia, Exploit <https://en.wikipedia.org/wiki/Exploit_(computer_security)>

[^108]: Wikipedia, Freedom Hosting <https://en.wikipedia.org/wiki/Freedom_Hosting>

[^109]: Wired, 2013 FBI Admits It Controlled Tor Servers Behind Mass Malware Attack <https://www.wired.com/2013/09/freedom-hosting-fbi/>

[^110]: Wikipedia, 2020 United States federal government data breach <https://en.wikipedia.org/wiki/2020_United_States_federal_government_data_breach>

[^111]: Wikipedia, Sandbox <https://en.wikipedia.org/wiki/Sandbox_(computer_security)>

[^112]: Wikipedia, CPU <https://en.wikipedia.org/wiki/Central_processing_unit>

[^113]: Wikipedia, Intel Management Engine <https://en.wikipedia.org/wiki/Intel_Management_Engine>

[^114]: Wikipedia, AMD Platform Security Processor <https://en.wikipedia.org/wiki/AMD_Platform_Security_Processor>

[^115]: Wikipedia, IME, Security Vulnerabilities <https://en.wikipedia.org/wiki/Intel_Management_Engine#Security_vulnerabilities>

[^116]: Wikipedia, IME, Assertions that ME is a backdoor <https://en.wikipedia.org/wiki/Intel_Management_Engine#Assertions_that_ME_is_a_backdoor>

[^117]: Wikipedia, IME, Disabling the ME <https://en.wikipedia.org/wiki/Intel_Management_Engine#Disabling_the_ME>

[^118]: Libreboot, <https://libreboot.org/>

[^119]: Digi.Ninja, Jasager <https://digi.ninja/jasager/>

[^120]: Hak5 Shop, Wi-Fi Pineapple <https://shop.hak5.org/products/wifi-pineapple>

[^121]: Wikipedia, Deautentication Attack <https://en.wikipedia.org/wiki/Wi-Fi_deauthentication_attack>

[^122]: Wikipedia, Capture Portal <https://en.wikipedia.org/wiki/Captive_portal>

[^123]: Wired, Why the Security of USB Is Fundamentally Broken <https://www.wired.com/2014/07/usb-security/>

[^124]: Wikipedia, Stuxnet <https://en.wikipedia.org/wiki/Stuxnet>

[^125]: Superuser.com, How do I safely investigate a USB stick found in the parking lot at work? <https://superuser.com/questions/1206321/how-do-i-safely-investigate-a-usb-stick-found-in-the-parking-lot-at-work>

[^126]: Magnet Forensics, Magnet AXIOM <https://www.magnetforensics.com/products/magnet-axiom/cloud/>

[^127]: Cellebrite, Unlock cloud-based evidence to solve the case sooner <https://www.cellebrite.com/en/ufed-cloud/>

[^128]: ArsTechnica, How the way you type can shatter anonymity---even on Tor <https://arstechnica.com/information-technology/2015/07/how-the-way-you-type-can-shatter-anonymity-even-on-tor/>

[^129]: Paul Moore Blog, Behavioral Profiling: The password you can\'t change. <https://paul.reviews/behavioral-profiling-the-password-you-cant-change/>

[^130]: EFF Panopticlick (<https://panopticlick.eff.org/>)

[^131]: ArsTechnica, Stakeout: how the FBI tracked and busted a Chicago Anon <https://arstechnica.com/tech-policy/2012/03/stakeout-how-the-fbi-tracked-and-busted-a-chicago-anon/>
...
[^133]: Chromium Documentation, Technical analysis of client identification mechanisms <https://sites.google.com/a/chromium.org/dev/Home/chromium-security/client-identification-mechanisms#TOC-Machine-specific-characteristics>

[^134]: Mozilla Wiki, Fingerprinting <https://wiki.mozilla.org/Fingerprinting>

[^135]: Facebook Research, Deepface <https://research.fb.com/publications/deepface-closing-the-gap-to-human-level-performance-in-face-verification/>

[^136]: Privacy News Online, Putting the "face" in Facebook: how Mark Zuckerberg is building a world without public anonymity <https://www.privateinternetaccess.com/blog/putting-face-facebook-mark-zuckerberg-building-world-without-public-anonymity/>

[^137]: CNBC, "Facebook has mapped populations in 23 countries as it explores satellites to expand internet" <https://www.cnbc.com/2017/09/01/facebook-has-mapped-human-population-building-internet-in-space.html>
...
[^139]: Brown Institute, Open Source Investigation, <https://brown.columbia.edu/open-source-investigation/>

[^140]: NewScientist, Facebook can recognize you in photos even if you\'re not looking <https://www.newscientist.com/article/dn27761-facebook-can-recognise-you-in-photos-even-if-youre-not-looking/>

[^141]: Google Patent, Techniques for emotion detection and content delivery <https://patents.google.com/patent/US20150242679>

[^142]: Slate <https://slate.com/technology/2018/04/facebook-collects-data-on-non-facebook-users-if-they-want-to-delete-it-they-have-to-sign-up.html>

[^143]: The Conversation <https://theconversation.com/shadow-profiles-facebook-knows-about-you-even-if-youre-not-on-facebook-94804>

[^144]: The Verge <https://www.theverge.com/2018/4/11/17225482/facebook-shadow-profiles-zuckerberg-congress-data-privacy>

[^145]: ZDNET <https://www.zdnet.com/article/anger-mounts-after-facebooks-shadow-profiles-leak-in-bug/>

[^146]: CNET <https://www.cnet.com/news/shadow-profiles-facebook-has-information-you-didnt-hand-over/>

[^147]: Anyvision <https://www.anyvision.co/>

[^148]: BBC, Met police deploy live facial recognition technology <https://www.theguardian.com/uk-news/2020/feb/11/met-police-deploy-live-facial-recognition-technology>

[^149]: YouTube, The Economist, China: facial recognition and state control \| The Economist <https://www.youtube.com/watch?v=lH2gMNrUuEY>

[^150]: Washington Post, Huawei tested AI software that could recognize Uighur minorities and alert police, report says <https://www.washingtonpost.com/technology/2020/12/08/huawei-tested-ai-software-that-could-recognize-uighur-minorities-alert-police-report-says/>

[^151]: The Intercept, How a Facial Recognition Mismatch Can Ruin Your Life <https://theintercept.com/2016/10/13/how-a-facial-recognition-mismatch-can-ruin-your-life/>

[^152]: IMDB, Gattaca 1997, <https://www.imdb.com/title/tt0119177/>

[^153]: IMDB, Person of Interest 2011 <https://www.imdb.com/title/tt1839578>

[^154]: IMDB, Minority Report 2002, <https://www.imdb.com/title/tt0181689>

[^155]: Joseph Steinberg, How To Prevent Facial Recognition Technology From Identifying You <https://josephsteinberg.com/how-to-prevent-facial-recognition-technology-from-identifying-you/>

[^156]: NIST, Face recognition accuracy with masks using pre-COVID-19 algorithms <https://nvlpubs.nist.gov/nistpubs/ir/2020/NIST.IR.8311.pdf>

[^157]: Wikipedia, Phishing <https://en.wikipedia.org/wiki/Phishing>

[^158]: Grayshirt, <https://www.grayshift.com/>

[^159]: Securephones.io, Data Security on Mobile Devices: Current State of the Art, Open Problems, and Proposed Solutions <https://securephones.io/main.pdf>

[^160]: Wikipedia, Gag Order, <https://en.wikipedia.org/wiki/Gag_order>

[^161]: Wikipedia, National Security Letter <https://en.wikipedia.org/wiki/National_security_letter>

[^162]: Heise Online (German), <https://www.heise.de/news/Gericht-zwingt-Mailprovider-Tutanota-zu-Ueberwachungsfunktion-4972460.html>

[^163]: PCMag, Did PureVPN Cross a Line When It Disclosed User Information? <https://www.pcmag.com/opinions/did-purevpn-cross-a-line-when-it-disclosed-user-information>

[^164]: Internet Archive, Wipeyourdata, "No logs" EarthVPN user arrested after police finds logs <https://archive.is/XNuVw#selection-230.0-230.1>

[^165]: Internet Archive, Invisibler, What Everybody Ought to Know About HideMyAss <https://archive.is/ag9w4#selection-136.0-136.1>

[^166]: Wikipedia, Lavabit Suspension and Gag order, <https://en.wikipedia.org/wiki/Lavabit#Suspension_and_gag_order>

[^167]: Wikipedia, Warrant Canary <https://en.wikipedia.org/wiki/Warrant_canary>

[^168]: Washington Post, The intelligence coup of the century <https://www.washingtonpost.com/graphics/2020/world/national-security/cia-crypto-encryption-machines-espionage/>

[^169]: Swissinfo.ch, Second Swiss firm allegedly sold encrypted spying devices <https://www.swissinfo.ch/eng/second-swiss-firm-allegedly-sold-encrypted-spying-devices/46186432>

[^170]: Wired, Mind the Gap: This Researcher Steals Data With Noise, Light, and Magnets <https://www.wired.com/story/air-gap-researcher-mordechai-guri/>

[^171]: Ben Nassi, Lamphone, <https://www.nassiben.com/lamphone>

[^172]: Defuse.ca, TrueCrypt\'s Plausible Deniability is Theoretically Useless <https://defuse.ca/truecrypt-plausible-deniability-useless-by-game-theory.htm>

[^173]: Privacy International, Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^174]: Wikipedia, TAILS, <https://en.wikipedia.org/wiki/Tails_(operating_system)>

[^175]: XKCD, Password Strength <https://xkcd.com/936/>

[^176]: QubesOS FAQ, <https://www.qubes-os.org/faq/#is-secure-boot-supported>

[^177]: Wikipedia, Secure Boot, <https://en.wikipedia.org/wiki/Unified_Extensible_Firmware_Interface#Secure_boot>

[^178]: Wikipedia, Booting <https://en.wikipedia.org/wiki/Booting>

[^179]: Wired <https://www.wired.com/2013/12/better-data-security-nail-polish/>

[^180]: Wikipedia, Virtual Machine <https://en.wikipedia.org/wiki/Virtual_machine>

[^181]: Sneak.Berlin , Your computer is not yours <https://sneak.berlin/20201112/your-computer-isnt-yours/>

[^182]: Thenextweb, Apple apps on Big Sur bypass firewalls and VPNs --- this is terrible <https://thenextweb.com/plugged/2020/11/16/apple-apps-on-big-sur-bypass-firewalls-vpns-analysis-macos/>

[^183]: Wikipedia, Plausible Deniability <https://en.wikipedia.org/wiki/Plausible_deniability>

[^184]: Privacytools.io, Don\'t use Windows 10 - It\'s a privacy nightmare <https://privacytools.io/operating-systems/#win10>

[^185]: Wikipedia, Key Disclosure Laws <https://en.wikipedia.org/wiki/Key_disclosure_law>

[^186]: Wikipedia, Bitlocker <https://en.wikipedia.org/wiki/BitLocker>

[^187]: Wikipedia, Evil Maid Attack <https://en.wikipedia.org/wiki/Evil_maid_attack>

[^188]: Wikipedia, Cold Boot Attack <https://en.wikipedia.org/wiki/Cold_boot_attack>

[^189]: CITP 2008 (<https://www.youtube.com/watch?v=JDaicPIgn9U>)

[^190]: ResearchGate, Defeating Plausible Deniability of VeraCrypt Hidden Operating Systems <https://www.researchgate.net/publication/318155607_Defeating_Plausible_Deniability_of_VeraCrypt_Hidden_Operating_Systems>

[^191]: SANS.org, Mission Implausible: Defeating Plausible Deniability with Digital Forensics <https://www.sans.org/reading-room/whitepapers/forensics/mission-implausible-defeating-plausible-deniability-digital-forensics-39500>

[^192]: SourceForge, Veracrypt Forum <https://sourceforge.net/p/veracrypt/discussion/technical/thread/53f33faf/>

[^193]: Microsoft, BitLocker Countermeasures <https://docs.microsoft.com/en-us/windows/security/information-protection/bitlocker/bitlocker-countermeasures>

[^194]: SANS, Windows ShellBag Forensics in-depth <https://www.sans.org/reading-room/whitepapers/forensics/windows-shellbag-forensics-in-depth-34545>

[^195]: University of York, Forensic data recovery from the Windows Search Database <https://eprints.whiterose.ac.uk/75046/1/Forensic_Data_Recovery_From_The_Windows_Search_Database_preprint_DIIN328.pdf>

[^196]: A forensic insight into Windows 10 Jump Lists <https://cyberforensicator.com/wp-content/uploads/2017/01/1-s2.0-S1742287616300202-main.2-14.pdf>

[^197]: BlackBag, Windows 10 Jump List Forensics <https://www.blackbagtech.com/blog/windows-10-jump-list-forensics/>

[^198]: Wikipedia, Gatekeeper <https://en.wikipedia.org/wiki/Gatekeeper_(macOS)>

[^199]: Wikipedia Veracrypt <https://en.wikipedia.org/wiki/VeraCrypt>

[^200]: OSTIF Veracrypt Audit, 2016, <https://ostif.org/the-veracrypt-audit-results/>

[^201]: Veracrypt Documentation, Unencrypted Data in RAM <https://www.veracrypt.fr/en/Unencrypted%20Data%20in%20RAM.html>

[^202]: Veracrypt Documentation, Data Leaks <https://www.veracrypt.fr/code/VeraCrypt/plain/doc/html/Data%20Leaks.html>

[^203]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Trim_(computing)>

[^204]: Veracrypt Documentation, Trim Operations <https://www.veracrypt.fr/en/Trim%20Operation.html>

[^205]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^206]: St Cloud State University, Forensic Research on Solid State Drives using Trim Analysis <https://repository.stcloudstate.edu/cgi/viewcontent.cgi?article=1141&context=msia_etds>

[^207]: WindowsCentral, Trim Tutorial <https://www.windowscentral.com/how-ensure-trim-enabled-windows-10-speed-ssd-performance>

[^208]: Veracrypt Documentation, Trim Operation <https://veracrypt.eu/en/docs/trim-operation/>

[^209]: Black Hat 2018, Perfectly Deniable Steganographic Disk Encryption <https://i.blackhat.com/eu-18/Thu-Dec-6/eu-18-Schaub-Perfectly-Deniable-Steganographic-Disk-Encryption.pdf>

[^210]: Milan Broz's Blog, TRIM & dm-crypt \... problems? <http://asalor.blogspot.com/2011/08/trim-dm-crypt-problems.html>

[^211]: Veracrypt Documentation, Rescue Disk <https://www.veracrypt.fr/en/VeraCrypt%20Rescue%20Disk.html>

[^212]: Wikipedia, Virtualbox <https://en.wikipedia.org/wiki/VirtualBox>

[^213]: Wikipedia, Whonix <https://en.wikipedia.org/wiki/Whonix>

[^214]: Oracle Virtualbox Manual, Snapshots <https://docs.oracle.com/en/virtualization/virtualbox/6.0/user/snapshots.html>

[^215]: Utica College, FORENSIC RECOVERY OF EVIDENCE FROM DELETED ORACLE VIRTUALBOX VIRTUAL MACHINES <https://programs.online.utica.edu/sites/default/files/Neal_6_Gonnella_Forensic_Recovery_of_Evidence_from_Deleted_Oracle_VirtualBox_Virtual_Machine.pdf>

[^216]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^217]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^218]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^219]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^220]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^221]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^222]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^223]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^224]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^225]: hCaptcha <https://www.hcaptcha.com/>

[^226]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^227]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^228]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^229]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^230]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>
...
[^232]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^233]: Developers Google Blog,
...
[^234]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^235]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^236]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^237]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^238]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^239]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^240]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^241]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^242]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^243]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^244]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^245]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^246]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^247]: Jumio, ID verification features <https://www.jumio.com/features/>

[^248]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^249]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^250]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^251]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^252]: Youwave, <https://youwave.com/>

[^253]: Bluestacks, <https://www.bluestacks.com/>

[^254]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^255]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^256]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^257]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^258]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^259]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^260]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^261]: ResearchGate, Live forensics method for acquisition on the Solid State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^262]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^263]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^264]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^265]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^266]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^267]: Blancco, Drive Eraser <https://www.blancco.com/products/drive-eraser/>

[^268]: Wikipedia, GParted <https://en.wikipedia.org/wiki/GParted>

[^269]: DBAN, <https://dban.org/>

[^270]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^271]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^272]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^273]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^274]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^275]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^276]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^277]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^278]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^279]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^280]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^281]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^282]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->
  ...
  [Your Printed papers:]: #your-printed-papers
  [Phishing:]: #phishing
  ...
  [hddsdd]: media/image24.jpeg {width="4.826388888888889in" height="2.7291666666666665in"}
  [1]: #hddsdd
  ...
  [3]: #linux-3
  [4]: #windows-3
  [How to securely delete specific files/folders on your HDD/SDD drives:]: #how-to-securely-delete-specific-filesfolders-on-your-hddsdd-drives
  ...
  [8]: media/image2.jpeg
  [9]: media/image3.jpeg
  [10]: media/image4.jpeg
  [11]: media/image5.jpeg
  [12]: media/image6.jpeg
  [13]: media/image7.jpeg
  [14]: media/image8.jpeg
  [15]: media/image9.jpeg
  [16]: media/image10.jpeg
  [17]: media/image11.jpeg
  [18]: media/image12.jpeg
  [19]: media/image13.jpeg
  [20]: media/image14.jpeg
  [21]: media/image15.jpeg
  [22]: media/image16.jpeg
  ...
  [23]: media/image17.jpeg
  [24]: media/image18.jpeg
  [25]: media/image19.jpeg
  [26]: media/image20.jpeg
  [27]: media/image21.jpeg
  [28]: media/image22.jpeg
  [https://ProtonMail.com/terms-and-conditions]: https://protonmail.com/terms-and-conditions
  [29]: media/image23.jpeg
  [30]: media/image25.jpeg
  [31]: media/image26.jpeg
