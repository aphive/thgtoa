...
Version 0.4.8 (draft), January 2021 (work in progress, some parts are incomplete) by AnonymousPlanet.
...
This technology is also widely used in CAPTCHAS[^230] services to verify that you are "human" and can be used to fingerprint a user.
...
-   If you absolutely want system wide plausible deniability[^189] despite the risks[^177].
...
Forensics could[^197]'[^200] use all those leaks[^208] to prove the existence of hidden data and defeat your attempts at using plausible deniability and to find out about your various sensitive activities.
...
-   Enter a strong passphrase (longer the better)[^180]
...
-   Use a strong passphrase[^180] for the Hidden Volume (obviously a different one than the one for the Outer Volume).
...
Also apply this fix from VirtualBox to mitigate Spectre[^224]/Meltdown[^225] vulnerabilities by running this command from the VirtualBox Program Directory:
...
Just use the provided Whonix Workstation VM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^226] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^227] authentication with the authenticator feature.
...
In itself Qubes is the best option in this guide for people who are more comfortable with Linux and tech in general. But it has some downsides such as the lack of OS wide plausible deniability and its hardware requirements and hardware compatibilities. While you can run this on 4GB of RAM as per their requirements[^228], the recommended RAM is 16GB. I would advise against using Qubes OS if you have less than 8GB of RAM.
...
You will need something to store your data (logins/passwords, identities and TOTP[^230] information).

For this purpose I strongly recommend KeePassXC because of their TOTP feature. This is the ability to create entries for 2FA[^231] authentication with the authenticator feature.
...
Captcha[^232] stands for "Completely Automated Public Turing test to tell Computers and Humans Apart" are Turing tests[^233] puzzles you need to complete before accessing a form/website. You'll mostly encounter those provided by Google (reCaptcha service[^234]) and Cloudflare (hCaptcha[^235]). hCaptcha is used on 15% of the internet by their own metrics[^236].
...
While most people think those puzzles are only about solving a little puzzle, it's important to understand that it's much more complex and that modern Captchas uses advanced machine learning and risk analysis algorithms to check if you're human[^237]:

-   They check your browser, cookies and browsing history using Browser fingerprinting[^238].

-   They track your cursor movements (speed, accuracy) and use algorithms to determine if it's "human".

-   They track your behavior before/during/after the tests to ensure you're "human"[^239].
...
Most platforms (including the privacy oriented ones such as Signal/Telegram/ProtonMail will require a phone number to register and most countries now make it mandatory to submit a proof of ID to register[^241].
...
Browser and Device[^242] Fingerprinting are usually integrated into the Captcha services but also in other various services.

Many platforms (like Google[^243]) will check your browser for various capabilities and settings and block Browsers they don't like. This is one of the reasons I recommend using Brave Browser over Tor Browser within your VM.
...
This is the part where you should watch the documentary "The Social Dilemma"[^244] on Netflix as they cover this topic much better than anyone else IMHO.
...
This is for instance now the case on YouTube for some European Users[^245].
...
This option is often presented as the "default one", hiding away the "Sign-in with e-mail and password" with clever Dark Patterns[^246] and unfortunately sometimes required.
...
This guide won't cover this one (yet) as it's mainly used on financial platforms (that will be able to identify you with other means anyway) and some dating apps like Tinder[^247]. Unfortunately this method is now also sometimes being used on Facebook[^248] and Instagram as part of their verification methods (tho I didn't face it yet so far).
...
This is the fun part where you will now create your identities from thin air. These identities do not exist but should be plausible and look "organic". They should ideally have a story ... a "legend" (yes this is actually the official term for this[^249])
...
-   Brave Browser (Chromium based) with a Private Tor Tab has (IMHO) a better acceptance level than Tor Browser (Firefox based). You will have less issues with captchas and online platforms[^241] if you use Brave than if you use Tor Browser (feel free to try this yourself).
...
-   Whonix/Tails: Create/Edit a file /usr/local/etc/torrc.d/50_user.conf[^250].

-   On Tor Browser: Edit the torrc file located at Browser/TorBrowser/Data/Tor[^251].
...
-   Can you create accounts through Tor? Yes but it's very difficult and their onion address[^252] won't help. In most cases you'll just have a random error at sign-up and your account suspended after sign-in.
...
If your account gets suspended [^255] (this can happen on sign-up, after signing-up or after using it in some Google services), you can still get it unsuspended by submitting[^256] an appeal/verification (which will again require your Phone number and possibly an e-mail contact with Google support with the reason). Suspension of the account does not disable the e-mail forwarding but suspended account will be deleted after a while.
...
LinkedIn will then require you go through a verification process that will unfortunately require you to send an ID proof (identity card, passport, driver license). This ID verification is processed by a company called Jumio[^257] that specializes in ID proofing. This is most likely a dead end as this would force you to develop some strong Photoshop skills.
...
They're recommended by privacytools.io[^258]. Their only apparent issue is that they do require (in most cases) a phone number or another e-mail address for registration (when you try to register from a VPN or Tor at least).

They claim they do not store/link the phone/e-mail associated with the registration but only store a hash that is not linked to the account[^259]. As long as their claim is true and the hash is not linked to your account, and that you followed my guide regarding the Burner phone and the pre-paid SIM card, you should be safe from tracking.
...
This time you will need to submit an appeal using a form[^260], provide a good reason and wait for the appeal to be processed by Twitter. During that process, it's possible that you will receive an e-mail (on ProtonMail) asking you to reply to a customer service ticket to prove that you do have access to your e-mail and that it's you. This will be directed toward your Gmail address but will arrive on your ProtonMail.
...
You're going to have to find a different way to post there using at least seven proxies[^261] that are not known by 4chan blocking system.
...
So the fact is that it's very very unlikely[^270]'[^271] for a forensic examiner to be able to recover data from a Trim enabled SSD but it's not completely impossible either[^272]'[^273]'[^274] if they're fast enough and have access to extensive equipment and skills.
...
Obviously for maximum security, Sanitize type options should be preferred. Unfortunately are no **free** easy (bootable with a menu) all-in-one tools available and you'll be left with either going with drive manufacturers provided tools, the free manual hdparm[^275] utility or going with a commercial tool such as PartedMagic[^276] or Blancco[^277].
...
This guide will therefore recommend the use of the free utility hdparm using a GParted[^278] boot USB.
...
Unfortunately Apple has removed the secure erase options from the trash bin even for HDD drives[^280]. So you're left with using other tools:
...
The best tool IMHO to remove metadata from various documents is the open-source mat2 recommended by privacytools.io[^281] (<https://0xacab.org/jvoisin/mat2>) which you can use on Linux/OSX quite easily. I never managed to make it work properly within Windows due various python dependencies despite the provided instructions. It's however very straightforward to install and use on Linux. So I would suggest creating a small Debian VM within Virtualbox (behind your Whonix Gateway) and then deploy the web version following the instructions provided here: <https://0xacab.org/jvoisin/mat2-web>. Which you can then use from your other VMs to analyze various files from a convenient web interface (I do recommend you take a snapshot of this VM before use to revert the state after analysis to remove traces from the VM in question).

Mat2 is also pre-installed on the Whonix Workstation VM[^282] and available on TAILS by default[^283].

Soon I intend to provide a convenient and ready-to-use Debian (stable) VM image with mat2-web pre-configured as an asset to this guide. This VM will work out of the box with Whonix Gateway and provide easy cross-platform access to Mat2 on OSX/Windows users.
...
Windows Event logs will keep many various pieces of information that could contain traces of your activities such as the devices that were mounted (including Veracrypt NTFS volumes for instance[^208]), your network connections, app crash information and various errors. It's always best to clean those up regularly. Do not do this on the Decoy OS.
...
As explained earlier, Shellbags are basically histories of accessed volumes/files on your computer. Remember that shellbags are very good sources of information for forensics[^200] and you need to clean those. Especially if you mounted any "hidden volume" anywhere. Again you shouldn't do this on the Decoy OS.
...
-   Download and install PrivaZer from <https://privazer.com/en/download.php>
    ...
    -   (if you did select Free Space cleaning) Within Clean Options **(Be careful with this option as it will erase all the free space on the selected partition, especially if you are running the decoy OS. Do not erase the free space or anything else on the second partition as you risk destroying your Hidden OS)**
        ...
        -   If you have an SSD drive:
            ...
            -   Secure Overwriting Tab: Personally I would just pick Normal Deletion + Trim (Trim itself should be enough[^212]). Secure Deletion with Trim[^209] (1 pass) might be redundant and overkill here if you intend to overwrite the free space anyway.
...
DuckDuckGo does not store cached version of pages[^284] and will instead forward you to a Google/Bing cached version if available.

In addition, DuckDuckGo source most of their searches from Bing (and not google)[^285] and therefore removing the content from Bing should in time have it removed it from DuckDuckGo too.
...
A commonly used technique which combines the idea of a Dead Drop[^286] and Secure Communication Obfuscation[^287] through Steganography[^288] and/or Kleptography[^289] and has many names such as Koalang[^290] or "Talking Around" or even "Social Steganography". This technique is very old and still widely used nowadays by teenagers to bypass parental control. It's hiding in plain sight.
...
As written earlier in this guide and as noted by Privacytools.io[^291], Windows 10 is a privacy nightmare. And disabling everything during and after the installation using the settings available to you is not enough. The amount of telemetry data collected by Microsoft is staggering and could defeat your attempts at keeping secrets. You will need to download and use a couple utilities to (hopefully) force Windows 10 into not sending data back to Microsoft.
...
-   The Windows USN journal which stores plenty of information[^293].

-   The Windows System Resource Usage Monitor (SRUM)[^294].

-   Various histories of various programs (such as the recent lists).

-   Various logs

-   The free (unallocated) space of your hard drive[^295].
...
There are several utilities that are recommend (like the old unmaintained DBAN[^296]) or System Rescue CD (<https://www.system-rescue.org/>) for this but personally, I will recommend the use of ShredOS.
...
[^224]: Wikipedia, Spectre <https://en.wikipedia.org/wiki/Spectre_(security_vulnerability)>

[^225]: Wikipedia, Meltdown <https://en.wikipedia.org/wiki/Meltdown_(security_vulnerability)>

[^226]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^227]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^228]: Qubes OS, System Requirements <https://www.qubes-os.org/doc/system-requirements/>

[^229]: Qubes OS Issues, Simulate Hibernation / Suspend-To-Disk \#2414 <https://github.com/QubesOS/qubes-issues/issues/2414>

[^230]: Wikipedia, TOTP <https://en.wikipedia.org/wiki/Time-based_One-time_Password_algorithm>

[^231]: Wikipedia, Multi-Factor Authentication <https://en.wikipedia.org/wiki/Multi-factor_authentication>

[^232]: Wikipedia, Captcha <https://en.wikipedia.org/wiki/CAPTCHA>

[^233]: Wikipedia, Turing Test <https://en.wikipedia.org/wiki/Turing_test>

[^234]: Google reCaptcha <https://www.google.com/recaptcha/about/>

[^235]: hCaptcha <https://www.hcaptcha.com/>

[^236]: hCaptcha hCaptcha Is Now the Largest Independent CAPTCHA Service, Runs on 15% Of The Internet <https://www.hcaptcha.com/post/hcaptcha-now-the-largest-independent-captcha-service>

[^237]: ArsTechnica, "Google's reCAPTCHA turns "invisible," will separate bots from people without challenges" <https://arstechnica.com/gadgets/2017/03/googles-recaptcha-announces-invisible-background-captchas/>

[^238]: BlackHat Asia 2016, "I'm not a human: Breaking the Google reCAPTCHA", <https://www.blackhat.com/docs/asia-16/materials/asia-16-Sivakorn-Im-Not-a-Human-Breaking-the-Google-reCAPTCHA-wp.pdf>

[^239]: Google Blog <https://security.googleblog.com/2014/12/are-you-robot-introducing-no-captcha.html>

[^240]: Cloudflare Blog, Cloudflare supports Privacy Pass <https://blog.cloudflare.com/cloudflare-supports-privacy-pass/>

[^241]: Privacy International,

    Timeline of SIM Card Registration Laws <https://privacyinternational.org/long-read/3018/timeline-sim-card-registration-laws>

[^242]: Wikipedia, Device Fingerprinting <https://en.wikipedia.org/wiki/Device_fingerprint>

[^243]: Developers Google Blog,
...
[^244]: IDMB, The Social Dilemma <https://www.imdb.com/title/tt11464826/>

[^245]: Google Help, Access age-restricted content & features <https://support.google.com/accounts/answer/10071085>

[^246]: Wikipedia, Dark Pattern <https://en.wikipedia.org/wiki/Dark_pattern>

[^247]: The Verge, Tinder will give you a verified blue check mark if you pass its catfishing test <https://www.theverge.com/2020/1/23/21077423/tinder-photo-verification-blue-checkmark-safety-center-launch-noonlight>

[^248]: DigitalInformationWorld, Facebook will now Require you to Create a Video Selfie for Identity Verification! [https://www.digitalinformationworld.com/2020/03/facebook-is-now-demanding-some-users-to-create-a-video-selfie-for-identity-verification.html\#]

[^249]: Wikipedia, Espionage, Organization <https://en.wikipedia.org/wiki/Espionage#Organization>

[^250]: Whonix Documentation, <https://www.whonix.org/wiki/Tor#Edit_Tor_Configuration>

[^251]: Tor Browser Documentation, <https://support.torproject.org/tbb/tbb-editing-torrc/>

[^252]: Facebook Onion Website <http://facebookcorewwwi.onion>

[^253]: Google Help <https://support.google.com/accounts/answer/114129?hl=en>

[^254]: Google Help <https://support.google.com/google-ads/answer/7474263?hl=en>

[^255]: Google, Your account is disabled <https://support.google.com/accounts/answer/40695>

[^256]: Google, Request to restore the account <https://support.google.com/accounts/contact/disabled2>

[^257]: Jumio, ID verification features <https://www.jumio.com/features/>

[^258]: Privacytools.io Recommended E-mail Providers <https://privacytools.io/providers/email/>

[^259]: ProtonMail Human Verification System [https://ProtonMail.com/support/knowledge-base/human-verification/]

[^260]: Twitter Appeal Form <https://help.twitter.com/forms/general>

[^261]: KnowYourMeme, Good Luck, I\'m Behind 7 Proxies <https://knowyourmeme.com/memes/good-luck-im-behind-7-proxies>

[^262]: Youwave, <https://youwave.com/>

[^263]: Bluestacks, <https://www.bluestacks.com/>

[^264]: Wikipedia, TPM <https://en.wikipedia.org/wiki/Trusted_Platform_Module>

[^265]: Wikipedia, Wear Leveling <https://en.wikipedia.org/wiki/Wear_leveling>

[^266]: Wikipedia, Trim <https://en.wikipedia.org/wiki/Write_amplification#TRIM>

[^267]: Wikipedia, Write Amplification <https://en.wikipedia.org/wiki/Write_amplification>

[^268]: Wikipedia, Trim Disadvantages <https://en.wikipedia.org/wiki/Trim_(computing)#Disadvantages>

[^269]: Wikipedia, Garbage Collection <https://en.wikipedia.org/wiki/Write_amplification#Garbage_collection>

[^270]: Techgage, Too TRIM? When SSD Data Recovery is Impossible <https://techgage.com/article/too_trim_when_ssd_data_recovery_is_impossible/>

[^271]: ResearchGate, Live forensics method for acquisition on the Solid State Drive (SSD) NVMe TRIM function <https://www.researchgate.net/publication/341761017_Live_forensics_method_for_acquisition_on_the_Solid_State_Drive_SSD_NVMe_TRIM_function>

[^272]: ElcomSoft, Life after Trim: Using Factory Access Mode for Imaging SSD Drives <https://blog.elcomsoft.com/2019/01/life-after-trim-using-factory-access-mode-for-imaging-ssd-drives/>

[^273]: Forensic Focus, Forensic Acquisition Of Solid State Drives With Open Source Tools <https://www.forensicfocus.com/articles/forensic-acquisition-of-solid-state-drives-with-open-source-tools/>

[^274]: ResearchGate, Solid State Drive Forensics: Where Do We Stand? <https://www.researchgate.net/publication/325976653_Solid_State_Drive_Forensics_Where_Do_We_Stand>

[^275]: Wikipedia, hdparm <https://en.wikipedia.org/wiki/Hdparm>

[^276]: Wikipedia, Parted Magic <https://en.wikipedia.org/wiki/Parted_Magic>

[^277]: Blancco, Drive Eraser <https://www.blancco.com/products/drive-eraser/>

[^278]: Wikipedia, GParted <https://en.wikipedia.org/wiki/GParted>

[^279]: UFSExplorer, Can I recover data from an encrypted storage? <https://www.ufsexplorer.com/solutions/data-recovery-on-encrypted-storage.php>

[^280]: EFF, How to: Delete Your Data Securely on MacOS <https://ssd.eff.org/en/module/how-delete-your-data-securely-macos>

[^281]: Privacytools.io, Productivity tools <https://www.privacytools.io/software/productivity/>

[^282]: Whonix Documentation, Scrubbing Metadata <https://www.whonix.org/wiki/Metadata>

[^283]: TAILS documentation, MAT <https://gitlab.tails.boum.org/tails/blueprints/-/wikis/doc/mat/>

[^284]: DuckDuckGo help, Cache <https://help.duckduckgo.com/duckduckgo-help-pages/features/cache/>

[^285]: DuckDuckGo help, Sources <https://help.duckduckgo.com/duckduckgo-help-pages/results/sources/>

[^286]: Wikipedia, Dead Drop <https://en.wikipedia.org/wiki/Dead_drop>

[^287]: Wikipedia, Secure Communication Obfuscation <https://en.wikipedia.org/wiki/Obfuscation#Secure_communication>

[^288]: Wikipedia, Steganography <https://en.wikipedia.org/wiki/Steganography>

[^289]: Wikipedia, Kleptography <https://en.wikipedia.org/wiki/Kleptography>

[^290]: Wikipedia, Koalang <https://en.wikipedia.org/wiki/Koalang>

[^291]: Privacytools.io, Operating Systems <https://privacytools.io/operating-systems/>

[^292]: Brave Support, What is a Private Window with Tor? <https://support.brave.com/hc/en-us/articles/360018121491-What-is-a-Private-Window-with-Tor->

[^293]: Medium.com, The Windows USN Journal <https://medium.com/velociraptor-ir/the-windows-usn-journal-f0c55c9010e>

[^294]: Medium.com, Digging into the System Resource Usage Monitor (SRUM) <https://medium.com/velociraptor-ir/digging-into-the-system-resource-usage-monitor-srum-afbadb1a375>

[^295]: SANS, Timestamped Registry & NTFS Artifacts from Unallocated Space <https://www.sans.org/blog/timestamped-registry-ntfs-artifacts-from-unallocated-space/>

[^296]: DBAN, <https://dban.org/>
